...
export async function renderNotifications(host, countEl) {
  injectAccountStyles();
  try {
    ...
    if (!followed.size) {
      host.innerHTML = '<p class="av-empty">Sigue o mira animes (❤️/🔖) para recibir avisos de episodios nuevos.</p>';
      return;
    }
    const cutoff = Date.now() - 30 * 86400000, weekAgo = Date.now() - 7 * 86400000;
    const notifs = [];
    animes.filter((a) => followed.has(a.id)).forEach((a) => {
      (a.episodes || []).forEach((ep) => {
        const dt = parseRelease(ep.releaseDate);
        if (dt && dt.getTime() >= cutoff) notifs.push({ animeId: a.id, animeTitle: a.title, img: ep.img || a.img, season: ep.season, number: ep.number, title: ep.title, dt });
      });
    });
    notifs.sort((x, y) => y.dt - x.dt);
    if (countEl) countEl.textContent = notifs.length || "";
    host.innerHTML = notifs.length
      ? notifs.slice(0, 40).map((n) => `<a class="av-ep" href="${epLink(n)}"><img src="${n.img}" loading="lazy" alt=""><div><b>${n.animeTitle}</b>${n.dt.getTime() >= weekAgo ? '<span class="av-new">NUEVO</span>' : ""}<br><small>Nuevo episodio · ${n.season} · E${n.number} ${n.title || ""}</small></div></a>`).join("")
      : '<p class="av-empty">Sin novedades recientes de tus animes.</p>';
